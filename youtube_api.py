...
class YouTubeAPI:
    ...
    def get_my_channels(self) -> List[Dict[str, Any]]:
        ...
        # Setup debug logging
        log_file = Path.home() / '.config' / 'yt-tui' / 'channels_debug.log'
        ...
        try:
            ...
            logger.info(f"=== CHANNELS DEBUG ===")
            logger.info(f"mine=True returned {len(channels)} channels")
            if channels:
                for ch in channels:
                    logger.info(f"  - {ch['title']} (ID: {ch['id']})")
            ...
            try:
                ...
                if channels:
                    logger.info(f"Fetching contentOwnerDetails for channel: {channels[0]['id']}")
                    ...
                    logger.info(f"contentOwnerDetails response: {channel_details}")
                    ...
                    if channel_details.get('items'):
                        ...
                        logger.info(f"contentOwnerDetails: {content_owner_details}")
                        logger.info(f"content_owner_id: {content_owner_id}")
                    else:
                        logger.info("No items in contentOwnerDetails response")

                # Now try managedByMe with content owner ID if available
                if content_owner_id:
                    logger.info(f"Trying managedByMe=True WITH onBehalfOfContentOwner={content_owner_id}")
                    ...
                    logger.info(f"Trying managedByMe=True WITHOUT onBehalfOfContentOwner (no content owner ID found)")
                ...
                logger.info(f"managedByMe returned {len(mgmt_response.get('items', []))} channels")

                for item in mgmt_response.get('items', []):
                    channel_id = item['id']
                    snippet = item['snippet']
                    logger.info(f"  - Managed channel: {snippet['title']} (ID: {channel_id})")

                    # Check if we already have this channel
                    if not any(ch['id'] == channel_id for ch in channels):
                        logger.info(f"    -> Adding (not a duplicate)")
                        statistics = item.get('statistics', {})
                ...
                logger.error(f"managedByMe failed with exception: {type(e).__name__}: {e}")
                import traceback
                logger.error(traceback.format_exc())

            logger.info(f"Final channel count: {len(channels)}")
            logger.info(f"===================\n")
            return channels
